...
# Shap score wrapper for iml package
# return matrix of shap score and mean ranked score list
shap.score.rank.iml <- function(model, X_train, y_train, n_samples = 100){
    
    require(iml)
    require(data.table)
    
    # Force X_train to data.frame
    X_train <- as.data.frame(X_train)
    
    # Create predictor object
    predictor = Predictor$new(model, data = X_train, y = y_train)
    
    # Get shapley values for train set
    shap_contrib <- list()
    
    # Check if method is classification 
    shapley_check = Shapley$new(predictor = predictor, 
                                x.interest = X_train[1, ], 
                                sample.size = n_samples)
    
    # Check if method is classification
    if('class' %in% names(shapley_check$results)){
        ...
        for(i in 1:nrow(X_train)){
            ...
            for(j in unique(shapley$results$class)){
                
                shapley_sub <- shapley$results[shapley$results$class == j, c('feature', 'phi')]
                shapley_i <-  setNames(data.frame(t(shapley_sub[,-1])), shapley_sub[,1])
                shap_contrib[[j]] <- rbind(shap_contrib[[j]], shapley_i)
                
            }
            
            cat(paste('\r Completed:', i, 'of', nrow(X_train)))
            
            mean_shap_score <- lapply(shap_contrib,
                                      function(x) {
                                          colMeans(abs(x))[order(colMeans(abs(x)), decreasing = T)]
                                      })
            
            out <- list()
            
            for(j in unique(shapley_check$results$class)){ 
                
                out[[j]] <- list(shap_score = shap_contrib[[j]],
                                 mean_shap_score = mean_shap_score[[j]])
        ...
        for(i in 1:nrow(X_train)){
            ...
            shapley_sub <- shapley$results[, c('feature', 'phi')]
            shapley_i <-  setNames(data.frame(t(shapley_sub[,-1])), shapley_sub[,1])
            shap_contrib[['reg']] <- rbind(shap_contrib[['reg']], shapley_i)
            
            cat(paste('\r Completed:', i, 'of', nrow(X_train)))
        }
        
        # Remove list
        shap_contrib <- shap_contrib[['reg']]
        
        mean_shap_score <- colMeans(abs(shap_contrib))[order(colMeans(abs(shap_contrib)), decreasing = T)]
        
        out <- list(shap_score = shap_contrib,
                    mean_shap_score = (mean_shap_score))
...
plot.shap.summary <- function(data_long, low = "#FFCC33", high = "#6600CC"){
